...
- **Una foto sacada de costado ya no queda acostada para siempre.** Si el navegador no llegaba a procesar la foto antes de subirla —scripts deshabilitados, o el compresor del navegador dándose por vencido, que es justo lo que pasa en los celulares con poca memoria— la foto viajaba tal cual salió de la cámara. El servidor la achicaba **ignorando la orientación y descartándola en el proceso**, así que quedaba acostada y ya no había forma de enderezarla: el dato que decía cómo mostrarla se había perdido. Afectaba tanto a los comprobantes de gastos como al escaneo de facturas en Compras, donde además la IA leía cualquier cosa. Verificado de punta a punta: la misma foto que antes devolvía un renglón suelto ($8.500) y una fecha inventada, ahora devuelve el total correcto ($26.741) y la fecha real.
...
### Lector con IA: ajustes de prompt

#### Técnico

- **Prompt del lector de comprobantes reforzado en tres puntos**, como red de seguridad: (1) el TOTAL nunca puede ser el SUBTOTAL ni un renglón suelto, con el control de que el importe elegido tiene que ser el mayor del comprobante; (2) las fechas argentinas son DÍA/MES/AÑO; (3) la descripción es QUÉ se compró, no el nombre del proveedor. **Ninguno corrige una falla medida:** las lecturas erróneas que motivaron estos agregados venían de un comprobante que el fixture de prueba tenía dado vuelta, no del prompt. Medido después contra la API real con una fecha ambigua (05/03/2026) y un comprobante con subtotal, IVA y total, el prompt anterior ya acertaba 5/5 en fecha y 5/5 en monto. Se conservan igual porque el prompt de Compras arrastra una regla análoga y ganada a pulso para `unit_price`, y los comprobantes reales son bastante más sucios que un fixture sintético. Verificado end-to-end: devuelve el total (26.741, no el subtotal 22.100), la fecha correcta, el resumen de los 4 ítems reales sin inventar ninguno, y matchea el proveedor pese a la tilde.
- **El prompt de `InvoiceExtractor` (Compras) también lleva ahora la regla de fecha y el control del total.** Se había medido contra un comprobante generado por código y acertaba 5/5, de donde se concluyó que no hacía falta. Los datos reales lo desmienten: entre las compras duplicadas de producción hay dos facturas donde el mismo papel, escaneado dos veces, dio la fecha con el día y el mes cambiados de lugar (`0004-00084240` como 06/02 y 02/06; el número correlativo prueba que la real es 02/06), y una donde el total se leyó con un cero de menos (3.800 en vez de 38.000). **Un comprobante sintético es demasiado prolijo para medir esto: da falsos verdes.**
