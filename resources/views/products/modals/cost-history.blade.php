<x-crud-modal name="product-cost-history" title="Historial de costo">
    <div class="space-y-3">
        ...
        <template x-if="!costHistory.loading && !costHistory.failed && costHistory.rows.length === 0 && costHistory.isResale">
        ...
        <template x-if="!costHistory.loading && !costHistory.failed && costHistory.rows.length === 0 && !costHistory.isResale">
            <p class="text-sm text-masa-madre">
                Todavía no hay producciones de este artículo. Se registra una cada vez que lo fabricás.
            </p>
        </template>
        ...
        <template x-if="!costHistory.loading && costHistory.rows.length > 0">
            <div class="border border-miga rounded-md divide-y divide-miga max-h-80 overflow-y-auto">
                <template x-for="row in costHistory.rows" :key="row.recorded_at + row.cost">
                    <div class="flex items-center justify-between gap-3 px-3 py-2 text-sm" :class="row.cancelled && 'opacity-50'">
                        <div class="min-w-0">
                            <div class="text-corteza">
                                <span class="text-[10px] font-medium rounded px-1 py-0.5"
                                    :class="{
                                        'bg-gray-100 text-gray-600': row.source === 'manual',
                                        'bg-sky-50 text-sky-700': row.source === 'compra',
                                        'bg-amber-50 text-amber-700': row.source === 'produccion',
                                    }"
                                    x-text="row.source_label"></span>
                                <span class="text-masa-madre text-xs ml-1" x-text="row.recorded_at"></span>
                                <span x-show="row.cancelled" class="text-[10px] text-red-500 ml-1">· anulada</span>
                            </div>
                            ...
                            <template x-if="row.production_url">
                                <a :href="row.production_url" class="text-xs text-horno hover:underline truncate block"
                                    x-text="`Producción de ${row.production_label}`"></a>
                            </template>
                        </div>
                        <span class="font-mono text-corteza whitespace-nowrap" :class="row.cancelled && 'line-through'" x-text="'$ ' + row.cost_formatted"></span>
                    </div>
                </template>
            </div>
        </template>
