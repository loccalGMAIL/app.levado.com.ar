...
<aside class="hidden sm:flex flex-col w-52 shrink-0 bg-masa-madre min-h-full">
    ...
    <nav class="flex-1 py-3 overflow-y-auto">
        ...
        {{-- Producción --}}
        <x-sidebar-group title="Producción" slug="produccion" :active="request()->routeIs('recipes.*', 'production.*', 'production-orders.*', 'production-requests.*', 'reparto.*')">
            @include('components.sidebar-item', [
            ...
            @can('manage-costs')
                @include('components.sidebar-item', [
                    'href'   => route('reparto.clientes.index'),
                    'label'  => 'Reparto',
                    'active' => request()->routeIs('reparto.*'),
                    'id'     => 'sidebar-reparto',
