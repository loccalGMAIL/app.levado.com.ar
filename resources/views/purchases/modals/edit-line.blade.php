<x-crud-modal name="line-edit" title="Editar ítem" :show="$errorsInEditLine">
    <form method="POST" :action="'/purchases/{{ $purchase->id }}/lines/' + editingLine?.id" class="space-y-4"
        ...
        x-effect="qty = parseFloat(editingLine?.quantity_purchased) || 0; price = parseFloat(editingLine?.unit_price) || 0; ivaRate = parseFloat(editingLine?.iva_rate ?? 0.21);">
        @csrf
        @method('PATCH')
        ...
        <div class="bg-miga/50 rounded-md p-3 space-y-2">
            ...
            <div class="flex items-center justify-between text-sm border-t border-miga pt-2">
                <span class="font-medium text-corteza">Subtotal c/IVA</span>
                <span class="font-mono font-medium text-corteza" x-text="'$ ' + fmt(subtotalIva)"></span>
            </div>
        </div>
