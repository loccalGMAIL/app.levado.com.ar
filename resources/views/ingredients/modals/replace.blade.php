<x-crud-modal name="ingredient-replace" title="Reemplazar en recetas" max-width="lg">
    <form method="POST"
        :action="replacing ? '{{ route('ingredients.replace', ['ingredient' => '__id__']) }}'.replace('__id__', replacing.id) : '#'"
        class="space-y-4"
        x-data="{
            toId: '',
            preview: null,
        ...
        <p class="text-sm text-masa-madre">
            ...
            por otro ingrediente en todas las recetas que lo usan. El ingrediente viejo no se borra.
        </p>
        ...
        <div>
            ...
            <select x-ref="toSelect" name="to_id" required
                ...
                <option value="">— Seleccioná el ingrediente sustituto —</option>
            ...
            <p x-show="toId && replacing && Number(toId) === replacing.id" x-cloak class="mt-1 text-xs text-red-600">
                Elegí un ingrediente distinto del que estás reemplazando.
            </p>
        </div>
        ...
        <template x-if="preview && !loading">
            <div class="bg-miga/50 rounded-md p-3 space-y-2 text-sm">
                <template x-if="preview.recipes.length === 0">
                    <p class="text-masa-madre">Este ingrediente no se usa en ninguna receta. Igual se puede reemplazar (por si vas a desactivarlo).</p>
                </template>
                <template x-if="preview.recipes.length > 0">
                    <div>
                ...
                <template x-if="preview.merges > 0">
                    <p class="text-masa-madre" x-text="preview.merges + ' línea(s) ya tenían el ingrediente destino y se fusionarán.'"></p>
                </template>
                <template x-if="preview.incompatible.length > 0">
                    <p class="text-red-600">
                        Unidad incompatible en: <span x-text="preview.incompatible.join(', ')"></span>.
        ...
        <label class="flex items-start gap-2 cursor-pointer select-none">
            ...
            <span class="text-sm text-masa-madre">
                <span class="font-medium text-corteza">Desactivar el ingrediente viejo</span>
                <span class="block text-xs">Ya no aparecerá para nuevas compras ni recetas.</span>
            </span>
        </label>

        <label class="flex items-start gap-2 cursor-pointer select-none">
            <input type="hidden" name="migrate_supplier_links" value="0">
            <input type="checkbox" name="migrate_supplier_links" value="1" checked
                class="mt-0.5 rounded border-gray-300 text-corteza focus:ring-horno">
            <span class="text-sm text-masa-madre">
                <span class="font-medium text-corteza">Migrar los vínculos con productos de proveedor</span>
                <span class="block text-xs">Para que la próxima factura escaneada asocie directo al ingrediente nuevo.</span>
            </span>
        </label>
        ...
        <div class="flex gap-3 pt-2">
            ...
            <button type="button"
                x-on:click="$dispatch('close-modal', 'ingredient-replace')"
