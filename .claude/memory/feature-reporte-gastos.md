...
- Modal `fixed-costs/modals/report.blade.php`: elige mes, cantidad de meses de histórico (6/12/24) y
  qué secciones incluir (vigentes, histórico mensual, detalle por gasto, gastos variables del
  período — opcional a pedido del usuario). Un checkbox "Aplicar los filtros de la pantalla" es
  **explícito**, no automático: un reporte que omite gastos por un filtro olvidado es un reporte
  que el contador firma mal.
...
`build(Tenant, array $options): array` compone 4 secciones sin duplicar SQL de los servicios ya
existentes:
- `current` — mismo `when(search)`/`when(status)` que `FixedCostController@index`, + monto real vía
  `FixedCostHistory::amountsForPeriod()` (no `monthly_amount` del modelo, que es sólo el vigente).
- `monthly` — **no reusa** `FixedCostHistory::monthlyTotals()`: esa ventana está anclada a
  `Carbon::now()` (útil para un dashboard), y el reporte necesita una ventana que termine en el
  período elegido (que puede ser un mes pasado). Reusa `totalForPeriod()` mes a mes en su lugar.
  Trampa real encontrada en desarrollo: el primer intento reusó `monthlyTotals()` + un filtro
  posterior `<= period`, que recorta mal la serie para cualquier período que no sea el mes actual.
- `details` — método nuevo `FixedCostHistory::timelinesFor(array $ids, Carbon $until, int $months)`:
  timelines de N gastos en una sola query (`whereIn` + `groupBy` en memoria) en vez de repetir
  `timelineFor()` gasto por gasto (N+1). El cálculo de `change_pct` se extrajo a un `private`
  compartido (`withChangePct()`) para que `timelineFor()` y `timelinesFor()` no diverjan. Tope de
  **50 gastos** (los de mayor monto), con footnote si se trunca.
- `variable` — `$tenant->variableExpenses()->between($from, $to)` ([[feature-compras]] no lo toca;
  ver `VariableExpense::scopeBetween`). Ojo: el row necesita `name` explícito además de
  category/supplier/description — se olvidó en el primer borrador y el test de esta sección lo
  agarró (`assertSee` sobre el nombre del gasto, no sobre su descripción).
...
## Tests
`tests/Feature/FixedCostReportTest.php`, 11 tests. Setup propio (`reportSetup()`) en vez de
