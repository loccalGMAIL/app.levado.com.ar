...
## v0.10.0 — Gastos Variables (2026-07-15)
- La pantalla de Gastos se dividió en dos pestañas: **Gastos Fijos** (sin cambios) y **Gastos Variables**, para registrar gastos ocasionales o imprevistos (una reparación, una compra puntual) que **no forman parte de los costos de producción**: no intervienen en el costo de recetas, márgenes ni ningún proceso productivo. Son registros administrativos, base para los reportes y el análisis financiero futuros (de ahí el índice `[tenant_id, expense_date]`).
- Un gasto variable es un **evento puntual**: nombre, categoría, fecha, monto único y **proveedor opcional**. Sin monto mensual, sin toggle activo/inactivo y sin historial de precios — nada de eso aplica a algo que pasó una sola vez. Catálogo de categorías propio, separado del de gastos fijos (decisión explícita del usuario). Tiene borrado; los gastos fijos no, porque borrarlos alteraría costos históricos.
- **Proveedor opcional** (`supplier_id` nullable + `nullOnDelete`), reutilizando el listado de proveedores existente y el modal compartido `suppliers/modals/quick-create.blade.php`. Es opcional porque una multa o un gasto bancario no tienen proveedor. El "+ Nuevo proveedor" **apila** el quick-create (`:z="60"`) como `purchases/modals/create.blade.php:72`, en vez de cerrar y reabrir el padre como ingredientes (que hace perder el formulario).
- **Trampa a recordar — el proveedor inactivo (regla general):** todo `<select>` de edición debe listar **todos** los proveedores (marcando `(inactivo)`), no sólo los activos, o el de un proveedor dado de baja no matchea, el select cae en la opción vacía, y guardar cualquier otro campo **borra el proveedor en silencio** (select opcional) o **bloquea el guardado** (select `required`). La solución: una sola query con todos, filtrada **en la vista** según el uso (alta = sólo activos, edición = todos, filtro = todos) — el filtro no puede ir en el controller porque el mismo `$suppliers` alimenta ambos modales. Se arregló en gastos variables, y en la misma versión también en **ingredientes, descartables y compras**, que ya lo sufrían con datos reales (compra #48 bloqueada; ingredientes "Azucar"/"Limon esc" perdiendo el proveedor). Con tests de regresión en las 4 suites, verificados reintroduciendo el bug a propósito.
- **Agujero de aislamiento corregido:** `Store`/`UpdateIngredientRequest` y `Store`/`UpdatePackagingRequest` validaban `supplier_id` con `exists:suppliers,id` **sin scope de tenant**, y sus controllers no tenían el `abort_unless` de `PurchaseController:81`. Como los IDs son seriales globales, un usuario podía asignarse un proveedor de otro negocio y ver su nombre en su listado (enumerando proveedores ajenos por ID). Ahora usan `Rule::exists(...)->where('tenant_id', ...)`. Confirmado con un test que falló primero.
- **Pendiente del mismo patrón:** `RecipeController:178-180` (`$ingredients`/`$packagings`/`$laborTypes` activos → `recipes/show`) y `PurchaseController:357-358` (líneas de compra). Una receta con una línea cuyo ingrediente se dio de baja podría sufrir lo mismo. Sin revisar.
- **Decisión de arquitectura:** tabla separada `variable_expenses`, **no** una columna `type` en `fixed_costs`. Motivo: el overhead se calcula con `$tenant->fixedCosts()->active()->sum('monthly_amount')` duplicado en 5 lugares (`DashboardController:50`, `RecipePriceController:43`, `RecipeController:182`, `BusinessController:19`, y un getter Alpine en `recipes/show.blade.php:112`). Con una columna `type` los 5 pasaban a necesitar `->where('type','fixed')`, y olvidar uno habría inflado silenciosamente el costo de todas las recetas sin que ningún test fallara. Con tabla separada el aislamiento es **estructural, no disciplinario**, y los 5 call sites quedaron intactos. Beneficio secundario: cero migración de datos (las filas existentes ya eran gastos fijos en la tabla de gastos fijos) y el paso 1 del onboarding (`AppServiceProvider:31`) sigue significando lo mismo sin tocarlo.
- **La pestaña es el selector de tipo.** El pedido original incluía un selector dentro del formulario, pero eso presuponía campos comunes; al divergir (`monthly_amount`+`active`+`valid_from` vs `amount`+`expense_date`), un selector tendría que mutar el formulario entero al cambiar. Consecuencia aceptada: un gasto no se puede convertir de un tipo a otro.
- Se descartó centralizar la fórmula del overhead en un `OverheadCalculator`: son 5 duplicados de código existente y correcto, y la tabla separada ya da la garantía. En su lugar, un test-ancla verifica que cargar gastos variables no mueve `overheadPerHour` ni el margen.
