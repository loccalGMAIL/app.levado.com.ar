...
- **Trampa a recordar — el proveedor inactivo:** el `<select>` de edición debe listar **todos** los proveedores (marcando `(inactivo)`), no sólo los activos, o un proveedor dado de baja no matchea, el select cae en "— Ninguno —" y guardar cualquier otro campo borra el proveedor en silencio. `ingredients/modals/edit.blade.php:36-43` **tiene ese bug** (preexistente, sin arreglar). La solución acá: una sola query con todos los proveedores, filtrada en la vista según el uso (alta = sólo activos, edición = todos, filtro = todos). Con test de regresión.
...
- 387 tests, todos verdes (35 nuevos). Gate de no-regresión: `FixedCostCrudTest` pasa 21/21 sin haber modificado el archivo.
