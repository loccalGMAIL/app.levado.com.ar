...
## Trabajo actual — v0.13.0 Artículos y Producción (rama `v0.13.0/articulos-produccion`, act. 04/09/2026)
Rama **no mergeada a master**. Módulo **product-céntrico**: el **Artículo** (`Product`) es el SKU vendible/stockeable y dueño de costo+precio; la **Receta** es su BOM. **Todo el detalle vive en [[feature-articulos-produccion]], [[domain-model-articulos]] y [[decision-multi-sucursal]]** — este bloque es solo el índice para retomar.
- **Hecho:** etapas 1–3 (catálogo de Productos, stock, compra de reventa, **Producción** backend+UI), P1/P2/P3 del modelo de dominio (costo y precio en el Artículo; políticas manual/margen/recargo), **Fase A** (matriz de precios dentro de Artículos + gestión de listas a Administración), **Fase B** (código EAN-13 interno para todos), **P4** (origen del costo visible + historial de costo de reventa + la reventa de primera clase en Compras), **P5** (**Órdenes de producción**: pedidos por destino —sucursal o repartidor—, resumen agregado + preview combinado, plantillas y repetir, planilla de reparto imprimible; repartidores como entidad nueva en Administración). Working tree con cambios sin commitear, **755 tests verdes**.
- **P5 verificado visualmente con el plugin de Chrome** (04/09/2026), contra la BD real de Confiteria Orfano: ciclo completo pedidos→agregado→preview→confirmar→producir→anular con datos reales, planilla de reparto, repetir orden y plantillas — todo correcto. De paso se encontró y arregló un bug real: `<x-responsive-table>` exige `mobileExpanded: false` en el `x-data` padre (documentado en el propio componente); `production-orders/index.blade.php` tenía `x-data="{}"` y mostraba card+tabla superpuestas. El mismo bug ya existía en `production/index.blade.php` (módulo viejo, nunca tocado antes) — nadie lo había notado porque esa pantalla nunca tuvo datos. Los botones Producir/Anular usan `confirm()` nativo, que congela el plugin de Chrome ~30s por click (patrón ya usado en toda la app) — se resuelve mandando `Return` tras el timeout. Quedaron datos de prueba a propósito en Orfano (categoría "Producción" en 2 alfajores, repartidor "Juan Reparto", 3 órdenes, la plantilla "Lunes de panadería") — el usuario pidió dejarlos.
- **Comandos de deploy nuevos** (orden importa): `products:from-recipes` → `products:backfill-prices` → `products:assign-codes` → `products:refresh-prices`. Ver la sección de deploy en [[feature-articulos-produccion]]. P4 sumó la tabla `product_cost_logs` (sin backfill); P5 sumó `production_orders`/`production_order_requests`/`production_order_lines`/`delivery_people` (todas arrancan vacías, sin backfill) + `npm run build`.
- **Próximo:** valuación por producción, semi-elaborados stockeables, movimientos de stock por destino (transferencias/reparto — diseñado sin construir en P5), mermas — sin elegir, están en [[feature-articulos-produccion]]. Después: Ventas/POS (probablemente cruce con clientes/reparto).
