...
## Trabajo actual — v0.13.1 Producción: costo con mano de obra, historial y alerta (rama `v0.13.1/produccion-ordenes`, act. 16/09/2026)
...
- **Hecho en esta rama (16/09/2026):** el costo de producción ahora incluye mano de obra (antes sólo insumos, quedaba más bajo que el del catálogo); historial de fabricaciones para el elaborado (badge **Receta** en el catálogo, mismo modal que ya tenía la reventa — sin tabla nueva, sale de `productions`); alerta de salto de costo al fabricar, comparando contra la producción anterior del mismo artículo. **Explícitamente NO se propagó `productions.unit_cost` al costo vigente del artículo** — se evaluó y se descartó, ver [[feature-articulos-produccion]] → "Punto 1 resuelto". **777 tests verdes** (22 nuevos). Pint corrido, sin commitear todavía.
- **Comandos de deploy** (orden importa): `products:from-recipes` → `products:backfill-prices` → `products:assign-codes` → `products:refresh-prices`. Ver la sección de deploy en [[feature-articulos-produccion]]. P4 sumó `product_cost_logs`; P5 sumó `production_orders`/`production_order_requests`/`production_order_lines`/`delivery_people`; esta rama sumó `productions.material_cost`/`labor_cost` + índice (con backfill de las producciones viejas). Todas requieren `npm run build` salvo la última.
- **Próximo (parkeado a pedido del usuario, no trabajar sin que lo pida):** semi-elaborados stockeables, movimientos de stock por destino (transferencias/reparto — diseñado sin construir en P5), mermas. Están en [[feature-articulos-produccion]]. Después: Ventas/POS (probablemente cruce con clientes/reparto).
