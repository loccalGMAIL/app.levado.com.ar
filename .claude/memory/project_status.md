...
- **Medir el contrafáctico antes de afirmar que un cambio de prompt arregla algo.** `git show <commit-viejo>:archivo > archivo`, correr N veces contra la API real, restaurar con `git checkout archivo`. Los cambios de prompt de v0.10.1 (regla del TOTAL, fecha DÍA/MES/AÑO, descripción ≠ proveedor) se conservan como red de seguridad, pero **no corrigen ninguna falla medida**: el prompt anterior daba 5/5 en fecha y monto.
- **El prompt de `InvoiceExtractor` (Compras) NO tiene el bug de fechas** que se sospechaba: medido 5/5 con una factura fechada 05/03/2026 (ambigua). El modelo infiere el formato argentino del contexto del prompt (panadería argentina, CUIT). No hace falta aclararle DÍA/MES/AÑO — **no reabrir esto sin medirlo primero**.
