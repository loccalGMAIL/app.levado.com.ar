...
- **Trampa del proveedor inactivo:** el `<select>` de edición lista **todos** los proveedores (marcando `(inactivo)`), no sólo los activos. Si listara sólo activos, un proveedor dado de baja no matchearía, el select caería en "— Ninguno —" y guardar cualquier otro campo borraría el proveedor en silencio. **`ingredients/modals/edit.blade.php:36-43` tiene exactamente ese bug** — no copiar ese patrón; hay test de regresión en `VariableExpenseCrudTest`.
...
- **387 tests, todos verdes** (35 nuevos, incluido el ancla de que los gastos variables no mueven el overhead y el de regresión del proveedor inactivo).
