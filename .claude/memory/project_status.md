...
## Trabajo actual — v0.13.1 Producción: numeración, Historial y Órdenes instantáneas (rama `v0.13.1/produccion-ordenes`, act. 16/09/2026)
Rama **no mergeada a master** (parte del mismo commit que cerró `v0.13.0/articulos-produccion`). Módulo **product-céntrico**: el **Artículo** (`Product`) es el SKU vendible/stockeable y dueño de costo+precio; la **Receta** es su BOM. **Todo el detalle vive en [[feature-articulos-produccion]], [[domain-model-articulos]] y [[decision-multi-sucursal]]** — este bloque es solo el índice para retomar.
- **Hecho (heredado de v0.13.0):** etapas 1–3 (catálogo de Productos, stock, compra de reventa, **Producción** backend+UI), P1/P2/P3 del modelo de dominio (costo y precio en el Artículo; políticas manual/margen/recargo), **Fase A** (matriz de precios dentro de Artículos + gestión de listas a Administración), **Fase B** (código EAN-13 interno para todos), **P4** (origen del costo visible + historial de costo de reventa + la reventa de primera clase en Compras), **P5** (**Órdenes de producción**: pedidos por destino, resumen agregado + preview combinado, plantillas y repetir, planilla de reparto; repartidores como entidad nueva).
- **Hecho en esta rama, primera tanda (costo/historial/alerta):** el costo de producción incluye mano de obra; historial de fabricaciones del elaborado (badge **Receta** en el catálogo); alerta de salto de costo al fabricar. **Explícitamente NO se propagó `productions.unit_cost` al costo vigente del artículo** — ver [[feature-articulos-produccion]] → "Punto 1 resuelto".
- **Hecho en esta rama, segunda tanda (16/09/2026):** numeración de órdenes ("Orden #N", por negocio) y pedidos ("Pedido N", dentro de su orden); pestaña **Historial** en Artículos (tabla global de producciones, reemplaza `/production`); **Orden instantánea** (un solo paso: destino + artículos → crea+confirma+produce, absorbe y retira "Producir suelto"). Ver [[feature-articulos-produccion]] → "Numeración, pestaña Historial y Órdenes instantáneas". **811 tests verdes** (34 nuevos sobre los 777 de la primera tanda). 6 commits, cada paso probado y comiteado por separado.
- **Comandos de deploy** (orden importa): `products:from-recipes` → `products:backfill-prices` → `products:assign-codes` → `products:refresh-prices`. Ver la sección de deploy en [[feature-articulos-produccion]]. P4 sumó `product_cost_logs`; P5 sumó `production_orders`/`production_order_requests`/`production_order_lines`/`delivery_people`; la primera tanda sumó `productions.material_cost`/`labor_cost` + índice; la segunda sumó `tenants.next_production_order_number` + `production_orders.number` + unique en `production_order_requests.position` (**con backfill de los datos reales existentes antes de cada unique** — verificar contra copia del dump antes de correr en producción). Todas con backfill requieren `npm run build` salvo la última.
- **Próximo (parkeado a pedido del usuario, no trabajar sin que lo pida):** semi-elaborados stockeables, movimientos de stock por destino (transferencias/reparto — condiciona si la orden instantánea mueve stock de verdad), mermas, plantilla recurrente con horizonte móvil vía cron (necesita `schedule:run` en el hosting, que hoy no corre). Están en [[feature-articulos-produccion]]. Después: Ventas/POS.
...
- **Etapa 3 (Artículos/Producción) ✅ hecha** en la rama v0.13.0 (ver el bloque "Trabajo actual" arriba). Sigue: **el Módulo de Producción** (alcance sin elegir) y luego **Ventas/POS**.
