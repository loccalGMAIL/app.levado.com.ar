...
## Trabajo actual — v0.13.0 Artículos y Producción (rama `v0.13.0/articulos-produccion`, act. 02/09/2026)
Rama **no mergeada a master** (43 commits sobre master; master es ancestro; historia lineal tras un rebase + un merge de master). Módulo **product-céntrico**: el **Artículo** (`Product`) es el SKU vendible/stockeable y dueño de costo+precio; la **Receta** es su BOM. **Todo el detalle vive en [[feature-articulos-produccion]], [[domain-model-articulos]] y [[decision-multi-sucursal]]** — este bloque es solo el índice para retomar.
- **Hecho:** etapas 1–3 (catálogo de Productos, stock, compra de reventa, **Producción** backend+UI), P1/P2/P3 del modelo de dominio (costo y precio en el Artículo; políticas manual/margen/recargo), **Fase A** (matriz de precios dentro de Artículos + gestión de listas a Administración), **Fase B** (código EAN-13 interno para todos), **P4** (origen del costo visible + historial de costo de reventa + la reventa de primera clase en Compras). Working tree limpio, **708 tests verdes**.
- **Comandos de deploy nuevos** (orden importa): `products:from-recipes` → `products:backfill-prices` → `products:assign-codes` → `products:refresh-prices`. Ver la sección de deploy en [[feature-articulos-produccion]]. P4 sumó la tabla `product_cost_logs`, que no necesita backfill.
- **Pendiente de verificación visual de P4**: la etiqueta de origen del costo y el modal de historial quedaron sin pasada en el navegador (la sesión de Chrome había expirado). Los tests cubren el render y el endpoint.
- **Próximo:** **Producción** — las cuatro direcciones (valuación por producción, semi-elaborados stockeables, órdenes, mermas) siguen sin elegir; están en [[feature-articulos-produccion]]. Después: Ventas/POS.
