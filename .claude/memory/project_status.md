...
- **Trampa del EXIF (aplica a cualquier captura de foto):** los teléfonos guardan la orientación en el EXIF, no en los píxeles, y `canvas.toBlob()` **descarta el EXIF** al re-encodear — la foto se subía acostada aunque en la galería se viera derecha. Hay que decodificar con `createImageBitmap(file, { imageOrientation: 'from-image' })` para hornear la orientación en los píxeles **antes** del re-encode. El default del estándar cambió con el tiempo, así que hay que declararlo explícito y no confiar en el navegador. Estaba mal desde siempre y afectaba también al escaneo de facturas.
