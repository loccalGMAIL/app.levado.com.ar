...
- **Trampas del prompt de visión, encontradas probando contra la API real** (valen para cualquier lector de comprobantes que se agregue): sin decírselo explícitamente, el modelo (1) toma el SUBTOTAL en vez del TOTAL — hay que darle el control "el importe elegido debe ser el mayor del comprobante"; (2) interpreta `14/05/2026` como enero, formato EE.UU. — hay que aclararle que las fechas argentinas son DD/MM/AAAA (el prompt de Compras tampoco lo aclara: **candidato a revisar en `InvoiceExtractor`**); (3) usa el nombre del proveedor como descripción del gasto.
- **Al verificar visión, mirar SIEMPRE la imagen que recibe la API antes de culpar al prompt.** Perdí dos rondas ajustando el prompt por lecturas alucinadas que en realidad venían de un comprobante dado vuelta 180° en el fixture de prueba. Con la imagen derecha, el mismo prompt acierta todo.
...
- **`InvoiceImagePreparer` (PHP/GD) NO mira el EXIF** (`imagecreatefromstring` lo ignora). Hoy no molesta porque el JS hornea la orientación antes de subir, pero `compressInvoiceImage` **devuelve el original ante cualquier error**: en un navegador donde el decode falle, al servidor le llega el JPEG con EXIF y lo guarda acostado. Comprobado a mano: mandando el archivo crudo al endpoint de scan, la IA lee cualquier cosa. Gap latente, sin arreglar.
