...
- [Listados con una sola fuente de render](pattern-listados-fuente-unica.md) — `x-data-table`: se escribe sólo el `<tr>` y el CSS decide si se ve como fila o como card. Roles de celda, las trampas de `grid-template-areas` y `:empty`, por qué no se usa `@aware`, y qué vistas faltan migrar (v0.12.10). Incluye la otra duplicación: el objeto Alpine repetido por fila → `Alpine.data()`, con las trampas del hoisting de `Alpine.start()` y del spread sobre getters, y el enum `MarginTier` como dueño único de los cortes de margen (v0.12.11)
